...
$(document).ready(function(){
  ...
  $('#data').on('click', 'tbody tr', function(){
      switch($('#data').attr('data-view-type')){
        case 'agencies':
          ...
          break;
        case 'routes':
          getStops($("#data").attr('data-agency-key'), $('.table_id', this).html());
     //     getStops(
          break;
        case 'stops':
          getStop.call(this);
  ...
  $('#top-menu')
	...
	$.getJSON('api/agencies', function(data){
		...
		return;
          ...
          $("#agencySelect").append('<option class="agencyOption" value="'+agency.agency_key+'">'+agency.agency_name+'</option>');
	      });
	    });
  ...
  //form handler
  $('#agencySelectForm').submit(function(e){
    e.preventDefault();
   // console.log("find routes");
    if($('#agencySelect:selected').val() != ''){
      $('#agencySelectForm fieldset').removeClass('error');
      //disable button
      //    $('#agencySelectForm input[type=submit]').attr('disabled', 'disabled');

      getRoutes($('.agencyOption:selected').val());
...
function getRoutes(key){
  var agency_key = key; // $(this).attr('data-agency-key');
  console.log(agency_key);
  $('#data').attr('data-view-type', 'routes');

  $('#map').hide();

  $('#data').attr('data-agency-key', key);
 // $('#nav-button').attr('data-previous', 'agencies');
  ...
  if(!agencies[agency_key].routes){
    ...
    $.getJSON('api/routes/' + agency_key, function(data){
      ...
      var reducedJSON = _.map(agencies[agency_key].routes, function(rt){
        return {
          'ID' : rt.route_id,
          'Name' : rt.route_long_name,
          'Description' : rt.route_desc,
          'Route Type' : rt.route_type,
          'Route URL' : rt.route_url,
          'Color': rt.route_color
        };
      });
   //   console.log(reducedJSON);
      renderTable(reducedJSON, 'routes');
...
function getStops(agency_key, route_id){
...
function renderTable(data, viewType){
  ...
  for(var key in data){
    ...
    columns.forEach(function(column, index){
      ...
      if(columns[index]=='Color' && viewType=='routes') {$(".table_id", row).css({'color':'#'+value, 'font-weight':'bold'});} //.append('<td><b><font color="#' + value + '">' + value + '</font></b></td>');}
      else if((columns[index]=='route_type' || columns[index] =="Route Type") && viewType=='routes') {$(row).append('<td>' + routeTypeFunction(value) + '</td>');}
      else if (index == 0 && viewType == 'routes') { $(row).append('<td class="table_id">' + value + '</td>'); }
